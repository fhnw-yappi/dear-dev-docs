...
\subsubsection{Yappi Chrome Extension}
    ...
    \paragraph{Benutzeroberfläche (UI)}
        ...
        \subparagraph{Systembenachrichtigungen}
            Beim Ende eines Meetings, erzeugt die Extension eine native Systembenachrichtigung siehe Abbildung \ref{fig:yappi-extension-notification}.
    ...
    \subparagraph{Popup-Ansicht}
        ...
        Die Ansicht enthält:
        \begin{itemize}
            \item Verbindungsstatus zum Backend (z.\,B. \texttt{Connected} mit grüner Statusanzeige),
            \item Benutzerhinweis zum erfolgreichen Setup,
            \item eine Liste kürzlich abgeschlossener Meetings mit Datum und Uhrzeit,
            \item Aktionsbuttons für jedes Meeting:
                ...
                \end{itemize}
        \end{itemize}
        Dieses Design stellt sicher, dass Nutzer sowohl über Echtzeitbenachrichtigungen als auch über die Popup-Ansicht
...
\section{Deployment und CI/CD}
    ...
    Das Deployment der \texttt{Yappi}-Plattform erfolgt automatisiert mittels \textit{Continuous Integration/Continuous Deployment}
    (CI/CD) über \textbf{GitHub Actions}. Die Ausführung erfolgt auf einem \textbf{SwitchEngines}-Cloud-Server mit \textbf{Debian Linux}.
    Der Betrieb basiert vollständig auf \textbf{Docker} und \textbf{Docker Compose}. Der externe Zugriff wird durch
    einen \textbf{NGINX}-Reverse-Proxy ermöglicht.

    \subsection{Build- und Deployment-Pipeline}

    Für Backend und Frontend bestehen separate GitHub-Actions-Workflows.
    Beide nutzen einen \textbf{self-hosted GitHub Runner}, der direkt auf dem Zielserver installiert ist.

        \subsubsection{Backend-Workflow}
            \begin{enumerate}
                \item Quellcode-Checkout aus dem GitHub-Repository.
                \item Einrichtung von \textbf{JDK 21} und Build des Backends mit Gradle.
                \item Anmeldung an der \textbf{GitHub Container Registry (GHCR)}.
                \item Erstellung und Upload des Backend-Docker-Images.
                \item Deployment:
                \begin{itemize}
                    \item Pull des aktuellsten Images.
                    \item Neustart der Container mit \texttt{docker compose}.
                \end{itemize}
            \end{enumerate}

        \subsubsection{Frontend-Workflow}
            \begin{enumerate}
                \item Quellcode-Checkout und Einrichtung von \textbf{Node.js} (Version 22.4.1).
                \item Ausführung des Linters und Build der \textbf{Next.js}-Anwendung.
                \item Erstellung und Upload des Frontend-Docker-Images in die GHCR.
                \item Deployment:
                \begin{itemize}
                    \item Pull des aktuellsten Images.
                    \item Neustart der Container mit \texttt{docker compose}.
            ...
            Die Ausführung der Workflows erfolgt automatisch bei Pushes auf die Branches \texttt{develop} und
            \texttt{main}. Zusätzlich kann der Prozess manuell über \texttt{workflow\_dispatch} gestartet werden.

    \subsection{Serverinfrastruktur}

        Der Betrieb erfolgt auf einem \textbf{SwitchEngines}-Cloud-Server mit 24/7-Verfügbarkeit.
        Als Betriebssystem wird \textbf{Ubuntu 24.04 LTS (Codename Noble)} eingesetzt.
        ...
        \begin{itemize}
            ...
            \item \textbf{Netzwerk}:
            \begin{itemize}
                \item Öffentliche IPv4-Adresse: \texttt{86.119.48.26}
            \end{itemize}
        \end{itemize}
        ...
        \begin{itemize}
            \item \textbf{NGINX-Reverse-Proxy} - leitet Anfragen an Backend (\texttt{/v1/}, \texttt{/apikey/}, \texttt{/companion/}) oder Frontend weiter und unterstützt CORS für Companion-Requests.
            ...
            \item \textbf{Frontend-Service} - Next.js-Anwendung für die Benutzeroberfläche.
    ...
    \subsection{NGINX-Proxykonfiguration}
        ...
        \begin{itemize}
            ...
            \item \texttt{/companion/} $\rightarrow$ Weiterleitung an das Backend mit speziellen CORS-Headern für Browser-Clients.
    ...
    Der \textbf{Eclipse Mosquitto}-Broker ist mit zwei Listenern konfiguriert:

    \begin{itemize}
        \item \textbf{TCP-Port 1883} für Backend-Services.
        \item \textbf{WebSocket-Port 9001} für Browser-Clients (z.\,B. die Chrome Extension).
    \end{itemize}

    Für den Prototypenbetrieb ist die Authentifizierung deaktiviert (\texttt{allow\_anonymous true}).

    \subsection{Zusammenfassung}

    Durch die Kombination aus GitHub Actions, Docker Compose und NGINX erfolgt der Betrieb der \texttt{Yappi}-Plattform weitgehend automatisiert. Codeänderungen werden unmittelbar getestet, gebaut und auf dem SwitchEngines-Server ausgerollt. Dies gewährleistet eine kontinuierliche und zuverlässige Auslieferung neuer Versionen.
